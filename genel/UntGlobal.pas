...
begin
  ...
  Edit;
    ...
    FieldByName('aciklama').AsString :='Üretim Ýçin Stok Çýkýþ ';
    FieldByName('Belgeno').AsString  :=BelgeNo;
    FieldByName('IslemTuru').AsString:='Stok Çýkýþ';
  ...
  Append;
    ...
    FieldByName('aciklama').AsString     := 'Üretim Ýçin Stok Çýkýþ';
    FieldByName('Belgeno').AsString      := BelgeNo;
    FieldByName('IslemTuru').AsString    := 'Üretim Çýkýþ';
    FieldByName('Birim').AsString        := UrunBirim;
...
with dm.Qry2 do
begin
  // Tanýmlý Depoyu bul... Üretim Deposunda mal Çekiliyor...
  Close;
  SQL.Clear ;
  SQL.Add('Select DepoKodu From ProjeDepolar where ProjeKod=:bul ');
  ...
  Edit;
    FieldByName('tarih').AsDateTime  :=Date;
    FieldByName('aciklama').AsString :='ARGE Ýçin Stok Çýkýþ ';
    FieldByName('Belgeno').AsString  :=BelgeNo;
    FieldByName('IslemTuru').AsString:='Stok Çýkýþ';
    FieldByName('Birim').AsString    :=UrunBirim;
    Cmiktar        :=Miktar;
  ...
  Append;
    FieldByName('tarih').AsDateTime      := Date;
    FieldByName('aciklama').AsString     := 'AR-GE Ýçin Stok Çýkýþ';
    FieldByName('Belgeno').AsString      := BelgeNo;
    FieldByName('IslemTuru').AsString    := 'ARGE Üretim Çýkýþ';
    FieldByName('Birim').AsString        := UrunBirim;
    FieldByName('Gmiktar').AsFloat       := 0;
    FieldByName('GBirimMaliyet').AsFloat := 0;
    FieldByName('GToplamMaliyet').AsFloat:= 0;
...
with dm.QryArama do
begin
  // *** stok tablosundaki deðerleri güncelle...
  Close;
  SQL.Clear;
...
try
  ...
  dm.cnn1.RollbackTrans;
  showmessage('Hesap Hareket Kayýt Hatasý..');
end;
end;
// ***
function Kur(Tip:String;Tarih:TDate;Sembol:string):String;
