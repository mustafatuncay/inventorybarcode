...
uses
  ...
  Dialogs, StdCtrls, ComCtrls,Comobj, Buttons, Gauges, Spin,StrUtils;
...
uses UntBarkodVeriAyar, UntSayimKontrol;
...
begin
   ...
   FrmBarkodVeriAyar.ComboBox1.Text:=Memo1.Lines[0];
   FrmBarkodVeriAyar.ComboBox2.Text:=Memo1.Lines[1];
   FrmBarkodVeriAyar.ComboBox3.Text:=Memo1.Lines[2];
   FrmBarkodVeriAyar.ComboBox4.Text:=Memo1.Lines[3];
   FrmBarkodVeriAyar.SpeedButton2Click(Sender);
   Erec1.text := '';
   Efd1.text := '';
   Erec2.text := '';
   Efd2.text := '';
...
begin
  memo1.Lines.LoadFromFile(ExtractFilePath(ParamStr(0))+'Ayar.set');
  FrmBarkodVeriAyar.ComboBox1.Text:=Trim(Copy(Memo1.Lines[0],pos('=',Memo1.Lines[0])+1,(Length(Memo1.Lines[0]))-pos('=',memo1.Lines[0])+1));
  FrmBarkodVeriAyar.ComboBox2.Text:=Trim(Copy(Memo1.Lines[1],pos('=',Memo1.Lines[1])+1,(Length(Memo1.Lines[1]))-pos('=',memo1.Lines[1])+1));
  FrmBarkodVeriAyar.ComboBox3.Text:=Trim(Copy(Memo1.Lines[2],pos('=',Memo1.Lines[2])+1,(Length(Memo1.Lines[2]))-pos('=',memo1.Lines[2])+1));
  FrmBarkodVeriAyar.ComboBox4.Text:=Trim(Copy(Memo1.Lines[3],pos('=',Memo1.Lines[3])+1,(Length(Memo1.Lines[3]))-pos('=',memo1.Lines[3])+1));
  FrmBarkodVeriAyar.ComboBox5.Text:=Trim(Copy(Memo1.Lines[4],pos('=',Memo1.Lines[4])+1,(Length(Memo1.Lines[4]))-pos('=',memo1.Lines[4])+1));
  FrmBarkodVeriAyar.ComboBox6.Text:=Trim(Copy(Memo1.Lines[6],pos('=',Memo1.Lines[6])+1,(Length(Memo1.Lines[6]))-pos('=',memo1.Lines[6])+1));
  FrmBarkodVeriAyar.Edit1.Text:=Trim(Copy(Memo1.Lines[9],pos('=',Memo1.Lines[9])+1,(Length(Memo1.Lines[9]))-pos('=',memo1.Lines[9])+1));
  FrmBarkodVeriAyar.ComboBox7.Text:=Trim(Copy(Memo1.Lines[7],pos('=',Memo1.Lines[7])+1,(Length(Memo1.Lines[7]))-pos('=',memo1.Lines[7])+1));
  FrmBarkodVeriAyar.ComboBox8.Text:=Trim(Copy(Memo1.Lines[8],pos('=',Memo1.Lines[8])+1,(Length(Memo1.Lines[8]))-pos('=',memo1.Lines[8])+1));
  FrmBarkodVeriAyar.ComboBox9.Text:=Trim(Copy(Memo1.Lines[5],pos('=',Memo1.Lines[5])+1,(Length(Memo1.Lines[5]))-pos('=',memo1.Lines[5])+1));
  FrmBarkodVeriAyar.Showmodal;
end;
...
procedure TFrmBarkodVeriAktar.SpeedButton4Click(Sender: TObject);
var i,x:integer;
TmpBarkod,TmpBarkod2:string;
begin

if CurrOdaNo.Value = 0 then begin
showmessage('Lütfen oda numarasýný giriniz ...');
exit;
end;
...
x:=0;
TmpBarkod := MemBarkod.Text ;

TmpBarkod2 := '';

for i:=1 to Length(TmpBarkod) do begin

TmpBarkod2 := TmpBarkod2 + TmpBarkod[i];

if (i mod 12)=0 then begin

FrmSayimKontrol.AdvTmp.Cells[0,x] := CurrOdaNo.Text;
FrmSayimKontrol.AdvTmp.Cells[1,x] := TmpBarkod2;


TmpBarkod2:='';
x:=x+1;
if i<>Length(TmpBarkod) Then FrmSayimKontrol.AdvTmp.AddRow;
end;
...
procedure TFrmBarkodVeriAktar.FormClose(Sender: TObject;
